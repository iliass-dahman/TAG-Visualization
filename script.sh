...
cqlsh cassandra -u cassandra -p cassandra -e "CREATE TABLE IF NOT EXISTS test.Card (id_card int PRIMARY KEY,end_of_validity text,id_client int,type text);"
cqlsh cassandra -u cassandra -p cassandra -e "CREATE TABLE IF NOT EXISTS test.Client (id_client int PRIMARY KEY, first_name text, last_index int, last_name text,top_line varchar);"
